...
const Footer = () => {
  const links = ["about", "ecosystem", "latest", "community"];
  const slides = [slide1, slide3, slide5, slide2, slide4, slide6];
  return (
    <footer className="flex flex-col gap-8 font-main bg-black text-white pt-24 overflow-hidden">
        ...
        <ul className="flex flex-col gap-2">
          {links.map((link) => (
            <li key={link} className="flex gap-2 items-center group/parent">
              ...
              <HashLink
                to={`#${link}`}
                className="font-light text-[16px] md:text-[18px] min-lg:text-[20px] xl:text-[22px] 2xl:text-[24px] group-hover/parent:bg-gray-700 rounded-2xl px-2 py-1 capitalize"
              >
                {link}
              </HashLink>
            </li>
          ...
          <li className="flex gap-2 items-center group/parent">
            ...
            <HashLink
              to={"#tokenomics"}
              className="font-light  text-[16px] md:text-[18px] min-lg:text-[20px] xl:text-[22px] 2xl:text-[24px] group-hover/parent:bg-gray-700 rounded-2xl px-2 py-1"
            >
              Tokenomics{" "}
            </HashLink>
          </li>
      ...
      >
        {slides.map((slide, index) => (
          ...
          >
            {index !== 0 && (
              <MdSubdirectoryArrowRight className="w-[50px] h-[50px] text-white" />
            )}
